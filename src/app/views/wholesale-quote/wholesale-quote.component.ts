...
@Component({
  selector: 'app-wholesale-quote',
  templateUrl: './wholesale-quote.component.html',
  styleUrls: ['./wholesale-quote.component.scss'],
  animations: egretAnimations,
})
export class WholesaleQuoteComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    var per_page_limit = localStorage.getItem('wholesale_quote_module_limit');
    var selected_order_dir = localStorage.getItem('wholesale_quote_module_order_dir');
    var selected_order_by = localStorage.getItem('wholesale_quote_module_order_by');
    var selected_page_no = localStorage.getItem('wholesale_quote_module_page_count');
    var search_keyword = localStorage.getItem('wholesale_quote_module_search_keyword');
      ...
      .pipe(
        ...
        tap(data => {
          if (!deepEqual(this.filter, data)) {
            if(data.search != ""){
              localStorage.setItem("wholesale_quote_module_search_keyword", data.search);
            }else{
              localStorage.removeItem("wholesale_quote_module_search_keyword");
            }
            this.filter = data;
            this.initFilter();
          }
  ...
  onPaginateChange(event) {
    ...
    localStorage.setItem('wholesale_quote_module_limit', event.pageSize);
    localStorage.setItem('wholesale_quote_module_page_count', data.page);
